...
namespace ShaderNoteD3D12.Commanding
{
    ...
    public class GraphicsCommandProxy
    {
        ...
        {
            if (!noteDevice.currentPSO.cbvDescriptions.TryGetValue(slot, out var desc))
                return 0;
            ...
            if (cbvs.TryGetValue(slot, out var cbv))
            {
                ...
                action(proxy);
                noteDevice.fastBufferAllocator.Upload(buffer, 256, out ulong addr);
                return addr;
            }
            return 0;
        }
        ...
        public void SetCBV(int slot, CBVAction action)
        {
            ...
            if (cbvs.TryGetValue(slot, out var cbv))
            {
                ...
                action(proxy);
                noteDevice.fastBufferAllocator.Upload(buffer, 256, out ulong addr);
                noteDevice.commandList.SetGraphicsRootConstantBufferView(cbv, addr);
            }
        }
        ...
        public void SetCBV(int slot, ulong gpuVirtualAddress)
        {
            if (cbvs.TryGetValue(slot, out var cbv))
            {
                noteDevice.commandList.SetGraphicsRootConstantBufferView(cbv, gpuVirtualAddress);
            }
        }
        ...
        public void SetCBV<T>(int slot, ReadOnlySpan<T> data) where T : unmanaged
        {
            if (cbvs.TryGetValue(slot, out var cbv))
            {
                noteDevice.fastBufferAllocator.Upload(MemoryMarshal.AsBytes(data), 256, out ulong addr);
                noteDevice.commandList.SetGraphicsRootConstantBufferView(cbv, addr);
            }
        }
        ...
        public void SetCBV<T>(int slot, T[] data) where T : unmanaged
        {
            if (cbvs.TryGetValue(slot, out var cbv))
            {
                noteDevice.fastBufferAllocator.Upload(MemoryMarshal.AsBytes<T>(data), 256, out ulong addr);
                noteDevice.commandList.SetGraphicsRootConstantBufferView(cbv, addr);
            }
        }
        ...
        public void SetSRV(int slot, ulong gpuVirtualAddress)
        {
            ...
            {
                noteDevice.commandList.SetGraphicsRootShaderResourceView(srv, gpuVirtualAddress);
            }
        }
        ...
        public void SetSRV(int slot, GpuDescriptorHandle handle)
        {
            if (srvs.TryGetValue(slot, out var srv))
            {
                noteDevice.commandList.SetGraphicsRootDescriptorTable(srv, handle);
            }
        }
        ...
        //    if (srvs.TryGetValue(slot, out var srv))
        //    {
        //        var handle = noteDevice.readonlyBufferAllocator.Upload(MemoryMarshal.AsBytes<T>(data), 256, out ulong addr);
        //        noteDevice.commandList.SetGraphicsRootDescriptorTable(srv, handle);
        //    }
        //}
        ...
        //    if (srvs.TryGetValue(slot, out var srv))
        //    {
        //        noteDevice.commandList.SetGraphicsRootShaderResourceView(srv, tlas.GPUVirtualAddress);
        //    }
        //}
        ...
        public void SetSRV(int slot, Texture2D texture, bool linear = false)
        {
            if (srvs.TryGetValue(slot, out var srv))
            {
                ...
                var handle = noteDevice.CreateSRV(texture.resource);
        ...
        //    if (srvs.TryGetValue(slot, out var srv))
        //    {
        //        var handle = noteDevice.GetSRVHandle(gpuBuffer);
        //        noteDevice.commandList.SetGraphicsRootDescriptorTable(srv, handle);
        //    }
        //}
        ...
        //            buffer.baseBuffer.ToState(noteDevice.commandList, ResourceStates.Common);
        ...
        //    if (srvs.TryGetValue(slot, out var srv))
        //    {
        //        noteDevice.commandList.SetGraphicsRootDescriptorTable(srv, graphicsContext.GetSRVHandleWithMip(texture, mips));
        //    }
        //}
        ...
        //        noteDevice.commandList.SetGraphicsRootDescriptorTable(uav, handle);
        //    }
        //}
        ...
        //    if (uavs.TryGetValue(slot, out var uav))
        //    {
        //        texture.SetPartResourceState(noteDevice.commandList, ResourceStates.UnorderedAccess, mipIndex, 1);
        ...
        //        noteDevice.commandList.SetGraphicsRootDescriptorTable(uav, graphicsContext.CreateUAV(texture.resource, uavDesc));
        //    }
        //}
        ...
        //    {
        //        var buffer = mesh.GetVertexBuffer(bufferName);
        //        buffer.baseBuffer.ToState(noteDevice.commandList, ResourceStates.UnorderedAccess);
